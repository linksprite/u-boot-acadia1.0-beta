...
#define GPIO_DOWNLOAD_KEY IMX_GPIO_NR(1, 8)
#define USB_OTG_PWR IMX_GPIO_NR(3, 22)
#define USB_H1_POWER IMX_GPIO_NR(1, 29)
#define GPIO_SATA_PWR IMX_GPIO_NR(4, 15)
...
*could not be power off anymore*/
	...
	setup_uart();
	//add by xiongwei sata power switch
	mxc_iomux_v3_setup_pad(MX6Q_PAD_KEY_ROW4__GPIO_4_15);
        gpio_direction_output(GPIO_SATA_PWR,1);
	if (cpu_is_mx6q())
		setup_sata();
...
//#ifdef CONFIG_ANDROID_RECOVERY
//fix by joe linksprite
int check_recovery_button(void)
{
	int button_pressed = 0;
	mxc_iomux_v3_setup_pad(MX6Q_PAD_GPIO_8__GPIO_1_8);
	gpio_direction_input(GPIO_DOWNLOAD_KEY);
	if (gpio_get_value(GPIO_DOWNLOAD_KEY) == 0) {
		udelay(100000);
		if (gpio_get_value(GPIO_DOWNLOAD_KEY) == 0) {
		button_pressed = 1;
...
iomux_v3_cfg_t display_pads[] = {
	...
	mxc_iomux_v3_setup_multiple_pads(display_pads, ARRAY_SIZE(display_pads));
}
...
static void setup_lcd_iomux(void)
{
	...
	/* LCD reset */
	mxc_iomux_v3_setup_pad(MX6Q_PAD_EIM_DA8__GPIO_3_8);
	/* Backlight */
	mxc_iomux_v3_setup_pad(MX6Q_PAD_SD1_DAT3__PWM1_PWMO);
#elif defined CONFIG_MX6DL
	/* LCD Power */
	...
	/* LCD reset */
	mxc_iomux_v3_setup_pad(MX6DL_PAD_EIM_DA8__GPIO_3_8);
	/* Backlight */
	mxc_iomux_v3_setup_pad(MX6DL_PAD_SD1_DAT3__PWM1_PWMO);
#endif
...
static void setup_hdmi_iomux(void)
{
	...
	mxc_iomux_v3_setup_multiple_pads(hdmi_i2c_pads, ARRAY_SIZE(hdmi_i2c_pads));
}
#endif
