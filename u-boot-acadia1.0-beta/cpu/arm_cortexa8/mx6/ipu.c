...
#define DI_TVE_COUNTER_BASECLK     1
#define DI_TVE_COUNTER_OHSYNC    2
#define DI_TVE_COUNTER_IVSYNC     3
#define DI_TVE_COUNTER_OVSYNC    4
#define DI_TVE_COUNTER_AFIELD     5
...
{
	unsigned int microCodeAddr_NL, microCodeAddr_EOL, microCodeAddr_NEW_DATA;
	int mapping = 1;
	...
	microcode_config(
	                 IPU, 
	                 microCodeAddr_NEW_DATA,//word address
	                 1,//stop
	                 "WROD",//OPCODE
	                 0, //LF
	                 0, //AF
	...
	**********************************************************************/
	microcode_event(IPU, DC_CHANNEL_DC_SYNC_OR_ASYNC, "NL",			3,	microCodeAddr_NL);  //prior=3, addr=2
	...
	microcode_event(IPU, DC_CHANNEL_DC_SYNC_OR_ASYNC, "NEW_DATA",	1,	microCodeAddr_NEW_DATA); 
	...
	* link events to routines
	**********************************************************************/
	microcode_event(IPU, DC_CHANNEL_DP_PRIMARY, "NL",			3,	microCodeAddr_NL);  //prior=3, addr=2
	...
	microcode_event(IPU, DC_CHANNEL_DP_PRIMARY, "NEW_DATA",	1,	microCodeAddr_NEW_DATA); 
	...
	ipu_write_field(IPU, IPU_DC_UGDE1_0__ODD_EN_1, 1);  // enable 'odd'
...
void di_config(unsigned int IPU, unsigned int DI)
{
	...
	**********************************************************************/
...
#endif
#else
	int ipuDiClk = 264000000;  //For iMX6, internel IPU clock, ipu_hsp_clk
#endif
	...
	di_sync_config(
		...
		0, // counter clear source selection
		1, // counter polarity
		1, // counter polarity trigger selection
		0, // counter polarity clear selection
		0, // counter up
	...
	di_sync_config(
		...
		0, // counter clear source selection
		1, // counter polarity
		DI_COUNTER_OHSYNC + 1, // counter polarity trigger selection
		0, // counter polarity clear selection
		0, // counter up
	...
	di_sync_config(
		...
		DI_COUNTER_OHSYNC + 1, // offset resolution
		0, // auto reload mode
		vDisp, // step repeat
		DI_COUNTER_OVSYNC + 1, // counter clear source selection
		0, // counter polarity
		0, // counter polarity trigger selection
		0, // counter polarity clear selection
		0, // counter up
	...
	if(DI == 0)
	{
		...
		ipu_write_field(IPU, IPU_DI0_SYNC_AS_GEN__DI0_VSYNC_SEL, DI_COUNTER_OVSYNC - 1);  //select PIN3 as VSYNC
#ifdef DI_CLOCK_EXTERNAL_MODE
	...
	else if(DI == 1)
	{
		...
		ipu_write_field(IPU, IPU_DI1_SYNC_AS_GEN__DI1_VSYNC_SEL, DI_COUNTER_OVSYNC - 1);  //select PIN3 as VSYNC
#ifdef DI_CLOCK_EXTERNAL_MODE
...
void ipu_setup(unsigned int IPU, unsigned int DI)
{
	...
	reg = readl(CCM_BASE_ADDR + 0x074);  // CCGR3
	reg &= ~0xFFFF;
	writel(reg, CCM_BASE_ADDR + 0x074);
...
#else
	if (DI == 0)
		clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK * 7, LVDS_DI0_CLK);
	else if(DI == 1)
		clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK * 7, LVDS_DI1_CLK);
#endif
	...
	{
		if(DI == 0)
			clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK, IPU1_DI0_CLK);
		else
			clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK, IPU1_DI1_CLK);
	}
	else if(IPU == 2)
	{
		if(DI == 0)
			clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK, IPU2_DI0_CLK);
		else
			clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK, IPU2_DI1_CLK);
	}
	...
	{
		if(DI == 0)
			clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK * 4, IPU1_DI0_CLK);
		else
			clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK * 4, IPU1_DI1_CLK);
	}
	else if(IPU == 2)
	{
		if(DI == 0)
			clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK * 4, IPU2_DI0_CLK);
		else
			clk_config(CONFIG_REF_CLK_FREQ, DISPLAY_PIX_CLOCK * 4, IPU2_DI1_CLK);
	}
	...
	{
		// Reset IPU
		reg = readl(SRC_BASE_ADDR + 0);  // SCR, SRC Control Register
		reg |= 0x8;
		writel(reg, SRC_BASE_ADDR + 0);
